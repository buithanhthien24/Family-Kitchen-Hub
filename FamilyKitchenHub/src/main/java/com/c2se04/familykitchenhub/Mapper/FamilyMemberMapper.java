...
import com.c2se04.familykitchenhub.DTO.AllergyResponseDTO;
import com.c2se04.familykitchenhub.DTO.FamilyMemberRequestDTO;
import com.c2se04.familykitchenhub.DTO.FamilyMemberResponseDTO;
...
public interface FamilyMemberMapper {
    ...
    @Mapping(target = "memberAllergies", ignore = true)
    ...
    // After mapping, convert MemberAllergy to AllergyResponseDTO
    @AfterMapping
    default void mapAllergies(@MappingTarget FamilyMemberResponseDTO responseDTO, FamilyMember entity) {
        if (entity.getMemberAllergies() != null && !entity.getMemberAllergies().isEmpty()) {
            Set<AllergyResponseDTO> allergyDTOs = entity.getMemberAllergies().stream()
                    .map(memberAllergy -> {
                        AllergyResponseDTO dto = new AllergyResponseDTO();
                        dto.setId(memberAllergy.getAllergy().getId());
                        dto.setName(memberAllergy.getAllergy().getName());
                        return dto;
                    })
