...
public class FamilyMemberService {
    ...
    public FamilyMember createFamilyMember(FamilyMember familyMember, Long userId, Set<Long> allergyIds) {
        ...
        // Validate and set Allergies through MemberAllergy if provided
        if (allergyIds != null && !allergyIds.isEmpty()) {
            Set<MemberAllergy> memberAllergies = new HashSet<>();
            for (Long allergyId : allergyIds) {
                ...
                MemberAllergy memberAllergy = new MemberAllergy(familyMember, allergy);
                memberAllergies.add(memberAllergy);
            }
            familyMember.setMemberAllergies(memberAllergies);
        }
    ...
    public FamilyMember updateFamilyMember(Long id, FamilyMember updatedDetails, Long userId, Set<Long> allergyIds) {
        ...
        // Update Allergies through MemberAllergy if provided
        if (allergyIds != null) {
            // Clear existing allergies
            existingMember.getMemberAllergies().clear();
            ...
            for (Long allergyId : allergyIds) {
                Allergy allergy = allergyRepository.findById(allergyId)
                        .orElseThrow(() -> new ResourceNotFoundException("Allergy", "id", allergyId));
                MemberAllergy memberAllergy = new MemberAllergy(existingMember, allergy);
                memberAllergies.add(memberAllergy);
            }
            existingMember.getMemberAllergies().addAll(memberAllergies);
        }
    ...
    public FamilyMember addAllergyToFamilyMember(Long memberId, Long allergyId) {
        ...
        boolean alreadyExists = member.getMemberAllergies().stream()
                .anyMatch(ma -> ma.getAllergy().getId().equals(allergyId));

        if (!alreadyExists) {
            MemberAllergy memberAllergy = new MemberAllergy(member, allergy);
            member.getMemberAllergies().add(memberAllergy);
        }
    ...
    public FamilyMember removeAllergyFromFamilyMember(Long memberId, Long allergyId) {
        ...
        // Remove the MemberAllergy association
        member.getMemberAllergies().removeIf(ma -> ma.getAllergy().getId().equals(allergyId));
